...
yl = forward('DNN_2_label', xl)
yn = forward('DNN_2_embed', xn)
yr = forward('DNN_2_embed', xr)
yt = tf.placeholder(tf.float32, label_shape, name="labeled_data_label")

with tf.name_scope("loss"):
    loss1 = supervised_loss('Cross_Entropy')(yl, yt)
    loss1 += graph_loss('UDP with Kernel Weighting')(yl_embed, yn, yr, wij_n, wij_r)* lambda_f
    loss_sum_1 = tf.summary.scalar('loss1', loss1[0][0])
    # loss += graph_loss('LE')(yl, yn, 1.0)
    # loss += graph_loss('LE')(yl, yr, 0.0)
    loss2 = graph_loss('UDP with Kernel Weighting')(yl_embed, yn, yr, wij_n, wij_r) * lambda_f
