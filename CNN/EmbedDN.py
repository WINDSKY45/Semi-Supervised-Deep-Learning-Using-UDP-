...
for i in range(no_epoch):
    ...
    _, loss_1 = sess.run(
        [train1,loss_sum_1],
        feed_dict={
            xl: xl_batch.reshape(-1, image_size, image_size, 1),
            xn: sample_data[xn_batch_index].reshape(-1, image_size, image_size, 1),
            xr: sample_data[xr_batch_index].reshape(-1, image_size, image_size, 1),
            yt: yt_batch,
            wij_n: wl_n,
            wij_r: wl_r
        })
    ...
    _, loss_2 = sess.run(
        [train2,loss_sum_2],
        feed_dict={
            xl: xu_batch.reshape(-1, image_size, image_size, 1),
            xn: sample_data[xun_batch_index].reshape(-1, image_size, image_size, 1),
            xr: sample_data[xur_batch_index].reshape(-1, image_size, image_size, 1),
            wij_n: wu_n,
            wij_r: wu_r
        })
    ...
    test_accuracy_sum = sess.run(
        acc_sum_1, feed_dict={
            xl: test_dataset.data.reshape(-1, image_size, image_size, 1),
